// ==UserScript==
// @name         Lichess Bot - PURE ALPHAZERO v4.3.1 ADAPTIVE DEFENSE - AUTHENTIC ECHO
// @description  AUTHENTIC AlphaZero Echo - Aggressive When Ahead, Solid Defense When Behind, BEATS STOCKFISH 8
// @author       Enhanced Human AI
// @version      4.3.1-ALPHAZERO-ADAPTIVE-DEFENSE
// @match         *://lichess.org/*
...
/**
 * ═══════════════════════════════════════════════════════════════════════════
 * PURE ALPHAZERO BOT v4.3.1 - AUTHENTIC ALPHAZERO ECHO - ADAPTIVE DEFENSE
 * ═══════════════════════════════════════════════════════════════════════════
 * 
 * CHANGELOG v4.3.1 (CRITICAL STABILITY FIX):
 * 🛡️ CRITICAL FIX: Added comprehensive error handling to detectPassedPawnDanger
 * 🛡️ CRITICAL FIX: Added safety checks for FEN validation
 * 🛡️ CRITICAL FIX: Quick exit when no pawns present (performance + stability)
 * 🛡️ ENHANCED: Try-catch wrapper prevents function from breaking bot
 * 🛡️ ENHANCED: Added currentFen validation before calling detection functions
 * 
 * PROBLEM SOLVED: Bot stopped moving in endgame (K+B vs K position)
 * SOLUTION: Robust error handling + validation checks + quick no-pawn exit
 * 
 ...
 * 8. Defensive Mode: Solid play when behind (eval < -100cp, risk 78%→40%)
 ...
 * 
 * Playing Style [AUTHENTIC ALPHAZERO ECHO - ADAPTIVE AGGRESSIVE/DEFENSIVE]:
 * - SOLID opening play - no hanging pieces/pawns!
 * - MAXIMUM explosive creativity when ahead/equal (35% base, up to 70% in complex)
 * - SOLID defensive play when behind (15% creativity, 40% risk tolerance)
 * - Deep calculation (depth 22-32 adaptive, +2 when behind or passed pawn danger)
 ...
 * - ADAPTIVE risk tolerance (78% when ahead, 40% when behind)
 * - Passed pawn danger detection and blockading
 * 
 * Core Principles (Authentic AlphaZero - Adaptive):
 ...
 * ✓ PASSED PAWN Detection and Stopping
...
const CONFIG = {
    ...
    // NEW: Defensive mode settings (when behind)
    ...
    defensiveCreativity: 0.15,       // Reduced creativity when behind (from 0.35)
...
function getStrategicDepth(phase, isStrategic, timeLeft) {
    ...
    // Only check if currentFen is valid
...
function applyAlphaZeroLogic(bestMove, alternatives) {
    ...
    if (isBehind) {
        ...
        // Even if score is close, only deviate rarely (15% creativity in defensive mode)
...
function initializeChessEngine() {
    ...
    console.log("🤖 Pure AlphaZero v4.3.1 ADAPTIVE DEFENSE - AUTHENTIC ECHO initialized");
    console.log("✅ STABILITY: Comprehensive error handling, robust pawn detection, safe FEN validation");
    console.log("✅ ADAPTIVE PLAY: Aggressive when ahead (creativity 70%, risk 78%), Solid when behind (creativity 15%, risk 40%)");
    console.log("✅ PASSED PAWN DETECTION: Stops advanced enemy pawns, depth +2 when danger detected");
    console.log("✅ DEFENSIVE MODE: Eval < -100cp triggers solid play, depth +2 for accuracy");
    console.log("✅ WINNING MODE: Aggressive conversion +100cp, positional sacrifices (45%)");
    console.log("✅ MOVE REJECTION RECOVERY: Auto-detects and recovers from rejected/illegal moves");
}
