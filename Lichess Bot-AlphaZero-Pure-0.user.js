 ...
 * ═══════════════════════════════════════════════════════════════════════════
 * 
 * CHANGELOG v4.4.0 (LEVEL 8 OPTIMIZATIONS - ENHANCED DEFENSIVE MASTERY):
 * 🎯 MAJOR: Defensive threshold LOWERED to -50cp (from -100cp) - Earlier defensive activation!
 * 🎯 MAJOR: Defensive creativity REDUCED to 5% (from 15%) - Maximum accuracy when behind!
 * 🎯 MAJOR: Defensive risk REDUCED to 20% (from 40%) - Extremely solid defensive play!
 * 🎯 MAJOR: Defensive depth bonus INCREASED to +5 (from +2) - Deeper calculation when defending!
 ...
 * 🎯 ENHANCED: Serious defensive mode (<-150cp) - ALWAYS plays best move, no exceptions!
 ...
 * SOLUTION: Enhanced passed pawn detection, earlier defensive mode, maximum solid play when behind
 * RESULT: Bot now detects and stops passed pawns much earlier, plays ultra-solid when behind!
 * TARGET: Specifically optimized to BEAT Lichess AI Level 8!
 * 
 ...
 * 
 * VALIDATION LAYERS (v4.4.0 ENHANCED):
 ...
 * 8. ENHANCED Defensive Mode: Ultra-solid play when behind (eval < -50cp, creativity 5%, risk 20%)
 * 9. ENHANCED Passed Pawn Detection: Advanced detection (rank 4+), depth +4 per pawn, +6 for multiple
 * 10. Serious Defensive Mode: Best move ONLY when eval < -150cp (no creativity allowed)
 * 
 ...
 * 
 * Playing Style [LEVEL 8 DESTROYER - ENHANCED DEFENSIVE MASTERY]:
 ...
 * - ULTRA-SOLID defensive play when behind (5% creativity, 20% risk tolerance)
 ...
 * - MAXIMUM defensive accuracy when behind (eval < -50cp, triggers EARLIER)
 * - Deepest long-term planning and strategic vision (95% focus)
 * - ADAPTIVE risk tolerance (78% when ahead, 20% when behind - MUCH MORE SOLID)
 * - Enhanced passed pawn detection (rank 4+) with severity scoring
 * - Special handling for multiple passed pawns (+6 depth)
 * 
 * Core Principles (Level 8 Destroyer - Enhanced Defense):
 * ✓ NO BLUNDERS in Opening
 * ✓ EXPLOSIVE Creativity in Balanced/Complex Positions
 * ✓ AGGRESSIVE Conversion when Winning
 * ✓ ULTRA-SOLID Defense when Behind (eval < -50cp, triggers MUCH EARLIER)
...
const CONFIG = {
    ...
    // NEW: Defensive mode settings (when behind) - ENHANCED for Level 8
    ...
    defensiveRiskTolerance: 0.20,    // Reduced risk when behind (from 0.40) - MORE SOLID
    defensiveCreativity: 0.05,       // Reduced creativity when behind (from 0.15) - MAXIMUM ACCURACY
...
function applyAlphaZeroLogic(bestMove, alternatives) {
    ...
    // ═══════════════════════════════════════════════════════════
    // NEW v4.3.0: DEFENSIVE MODE - ENHANCED for Level 8
    // ═══════════════════════════════════════════════════════════
    const isBehind = topScore < CONFIG.defensiveThresholdMild;
    const isLosingBadly = topScore < CONFIG.defensiveThresholdSerious;
    
    if (isBehind) {
        debugLog("[ENGINE]", `🛡️ DEFENSIVE MODE (${topScore}cp) - Maximum solid play`);
        
        // ENHANCED: Losing badly = ALWAYS play best move, no exceptions
        if (isLosingBadly) {
            debugLog("[ENGINE]", `🛡️🛡️ SERIOUS DEFENSIVE MODE (${topScore}cp) - Best move ONLY`);
            return bestMove;
        }
        
        // In defensive mode, ALWAYS play best move for accuracy
        // Exception: If there's a forced tactic that's nearly equal
        if (scoreDiff > 10) { // LOWERED from 15 for more accuracy
            debugLog("[ENGINE]", "🛡️ Clear best defense - no deviation");
            return bestMove;
        }
        
        // ENHANCED: Even if score is close, only deviate extremely rarely (5% creativity)
        if (Math.random() > CONFIG.defensiveCreativity) {
            debugLog("[ENGINE]", "🛡️ Defensive accuracy - playing best move");
            return bestMove;
        }
        
        // If we do deviate, only consider if alternative is almost as good
        if (scoreDiff < 5 && validateMoveForPosition(alternatives[1].move, currentFen)) { // LOWERED from 10
...
function initializeChessEngine() {
    ...
    console.log("🤖 Pure AlphaZero v4.4.0 LEVEL 8 DESTROYER - ENHANCED DEFENSE initialized");
    console.log("🎯 TARGET: Optimized specifically to BEAT Lichess AI Level 8!");
    console.log("✅ ENHANCED DEFENSE: Eval < -50cp triggers ultra-solid play (creativity 5%, risk 20%)");
    console.log("✅ DEEPER CALCULATION: Base depth 24, endgame depth 32, +5 depth when behind");
    ...
    console.log("✅ SERIOUS DEFENSIVE MODE: Eval < -150cp = best move ONLY, no creativity");
    console.log("✅ AGGRESSIVE WINNING: Creativity 70%, risk 78% when ahead, sharp conversions");
    console.log("✅ STABILITY: Comprehensive error handling, robust detection, move rejection recovery");
}
